...
Definition iperm (s : seq nat) (t : transposition) := t.
...
Lemma perm_eq_take_aperm m s i1 i2 
     (lti2m1s : i2 + m.+1 < size s) (le12 : i1 <= i2) : 
...
- have szap : n.+1 + m.+1 < size (aperm s (iperm s (ix, n.+1))). 
  ...
  move: (@IH (aperm s (iperm s (ix, n.+1))) m.+1 szap).
...
- set p := (iperm _ _); set s' := (aperm _ _).
  have [] := boolP (j <= n) => lejn.
...
Lemma aperm_id s n : aperm s (iperm s (n, n)) = s. 
Proof.
apply: (@eq_from_nth _ 0) => [|i ltis /=]; first by rewrite size_aperm.
rewrite size_aperm in ltis. 
by rewrite (nth_map 0) ?size_iota // nth_iota //= add0n /iperm toggle_id.
Qed.

Lemma nth_aperm s i x (ltns : i < size s) (xins : x \in s) :
  nth 0 (aperm s (iperm s (index x s, i))) i = x.
Proof. 
rewrite (nth_map 0) // /toggle /iperm /= ?size_iota ?nth_iota // add0n.
case: ifP => [/eqP -> |]; first by rewrite nth_index.
by rewrite ifT // nth_index.
Qed.

Lemma nth_aperm_id s i1 i2 i 
  (ltjs : i < size s) (lt1j : i1 < i) (lt2j : i2 < i) : 
  nth 0 (aperm s (iperm s (i1, i2))) i = nth 0 s i.
Proof.                                                                     
by rewrite /aperm (nth_map 0) // /toggle /iperm /= ?size_iota // nth_iota // add0n ifF ?gtn_eqF.
Qed.

Lemma perm_eq_aperm s i1 i2
  (lt1s : i1 < size s) (lt2s : i2 < length s) :
  perm_eq s (aperm s (iperm s (i1, i2))).
Proof.
apply/(perm_iotaP 0).
