...
Proof.
by move=> x y /eqP; rewrite (tnth_uniq (inord 0)) ?ouniq // => /eqP.
Qed.
...
case: (s1 == s) / eqP => [<-|/eqP neq_s1].
  ...
  by rewrite tnth_mktuple [in LHS]eqs1t => /o_injective/perm_inj->.
case: (s2 == s) / eqP => [<-|/eqP neq_s2].
  ...
  by rewrite tnth_mktuple [in LHS]eqs2t => /o_injective/perm_inj->.
...
by rewrite tnth_mktuple [in LHS]eqst => /o_injective/perm_inj->.
Qed.
...
Definition sorted_o o := 
  ∀ s1 s2 : slot, s1 <= s2 → 'idxa (tnth o s1) <= 'idxa (tnth o s2).
...
Lemma leq_bid (i1 i2 : A) :
  'idxa i1 <= 'idxa i2 -> 'bid_i2 <= 'bid_i1.
Proof.
move=> lti1I2.
...
rewrite !cancel_slot.  
have aioStar: tnth oStar s \in oStar by exact: mem_tnth.
rewrite aioStar !leq_mul2r. 
apply/orP; right.
...
Definition bidSum := @G.bidSum O_finType (biddings bs).

Lemma bidSumE o : bidSum o = \sum_(b <- biddings bs) b o.
Proof. by rewrite /bidSum G.bidSumE. Qed.
...
rewrite (bigID (fun j : A => j \in o)) bidSum_out_o /= addn0.
rewrite -(@big_tuple _ _ _ _ _ _ predT (fun j => bidding bs j o)).
by rewrite big_uniq ?ouniq => //=.
Qed.
...
have max_oStar (o : O) : predT o → geq (bidSum (OStar.oStar bs)) (bidSum o).
  move=> _.
  rewrite bidSum_slot /bidSum /G.bidSum // !bidsSum_sumBid.
...
Definition bidSum_i := @G.bidSum_i O_finType i (biddings bs).

Lemma bidSum_i_bid_ctr_agent : bidSum_i oStar = bidSum oStar - bid_ctr_agent i.
Proof.
rewrite /bidSum_i /G.bidSum_i /bidSum /G.bidSum.
...
Definition VCG_bidSum'' := G.bidSum'' i (biddings bs).

Lemma argmax_bidSum_oS'' (oS : O) (lew : forall o, welfare_i o <= welfare_i oS) :
  VCG_bidSum'' [arg max_(o > o0) VCG_bidSum'' o] = VCG_bidSum'' oS.
Proof.
apply/anti_leq/andP; split; last by rewrite -bigmax_eq_arg ?leq_bigmax.
rewrite -bigmax_eq_arg //.
apply/bigmax_leqP => o _.
rewrite /VCG_bidSum'' /G.bidSum''.
have body o': \sum_(j < n) tnth (G.bs'' i (biddings bs)) j o' =
              \sum_(j < n) t_bidding_i j o'.
  apply: eq_bigr => j _.
  by rewrite !tnth_mktuple !ffunE /t_bidding_i. 
...
have bidSum_out_o_i (o' : O) :
  ...
  case: (j != i) => //.
  by rewrite ifN.
rewrite (bigID (fun j : A => j \in o)) bidSum_out_o_i.
rewrite [X in _ <= X](bigID (fun j : A => j \in oS)) bidSum_out_o_i.
rewrite /= !addn0.
...
Definition welfare_with_i := @G.welfare_with_i O_finType o0 i (biddings bs).
...
under (eq_bigl (fun j => true && (j != i))) => j. 
  by rewrite andTb.
...
apply/negPf. 
apply: gtn_eqF.
by rewrite (@leq_ltn_trans k') // ?leq_ord // not_in_oStar_inv.
Qed.
...
Let bidSum' := @G.bidSum O_finType (biddings bs'). 

Let eq_welfare_without_i''_bidSum' : welfare_without_i'' = bidSum' oStar.
Proof.
rewrite /welfare_without_i'' /G.welfare_without_i'' /bidSum'.
rewrite (@bigmax_eq_arg _ o0 _ (fun o => G.bidSum'' i (biddings bs) o)) //. 
move: argmax_bidSum''; rewrite /VCG_bidSum'' => ->.
rewrite /G.bidSum'' /G.bidSum.
apply: eq_bigr => j _.
rewrite /G.bs'' tnth_map ffunE tnth_ord_tuple. 
have [] := boolP (j != i) => [neji|] //.
rewrite negbK => /eqP ->.
...
Proof.
rewrite eq_welfare_without_i''_bidSum' /bidSum' /G.bidSum.
rewrite bidsSum_sumBid -valid_bidSum bidSum_slot.
apply: eq_bigr => j _.
rewrite /bidding ffunE /bidding /t_bidding.
rewrite /bid_ctr_slot !tnth_map !tnth_ord_tuple.
...
- apply: eq_big => s; first by rewrite -leqNgt.
  ...
  by rewrite shifted_tuple_i tnth_mktuple -?widen_slot_as_agent //= lt_slot_n'.
Qed.
...
Proof.
rewrite /price tsortK //; last by exact: sorted_bids_sorted. 
...
Proof.
by rewrite !ffunE /t_bidding /bid_in !(idxaK sorted_bids_sorted) !(tsortK sorted_bids_sorted).
Qed.

Lemma sorted_relabelled_bidding j o': instance_bidding bs' j o' = bidding bs' j o'.
Proof.
have sortedbs' := sorted_bids_sorted.
rewrite !ffunE /t_bidding.
congr (if _ then _ else _); first by rewrite idxaK.
...
rewrite !tnth_mktuple.
apply/ffunP => o'.
by rewrite sorted_relabelled_bidding.
Qed.
...
rewrite !tnth_mktuple.
apply/ffunP => o'.
rewrite -relabelled_bidding. 
by rewrite permE -{1}(@cancel_inv_idxa _ _ geq_bid bs j).
Qed.  
...
have mkES o : 
  ...
  move=> [ox pT ge].
  have tge (o' : O_finType) (p : predT o') :
    geq (G.bidSum (biddings bs') ox) (G.bidSum (biddings bs') o').
    move: (ge o' pT).
    by rewrite !(G.perm_bidSum instance_perm_biddings) sorted_relabelled_biddings.
  exact: (ExtremumSpec pT tge). 
